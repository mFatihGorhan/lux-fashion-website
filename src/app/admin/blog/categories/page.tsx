...
import { Plus, Edit2, Trash2, Search, Hash, FileText } from 'lucide-react'
...
export default function BlogCategoriesPage() {
  const [data, setData] = useState<BlogCategoriesResponse | null>(null)
  ...
  const fetchCategories = async () => {
    try {
      ...
      const response = await fetch(`/api/admin/blog/categories?${params}`)
      if (response.ok) {
  ...
  const handleDelete = async (id: string) => {
    if (!confirm('Bu blog kategorisini silmek istediğinizden emin misiniz?')) return
  ...
  if (loading) {
    return (
      <div style={{ 
        ...
        <div>Blog kategorileri yükleniyor...</div>
      </div>
    )
  }
      ...
      <div style={{ 
        ...
        <div>
          ...
          <p style={{ color: '#9CA3AF', margin: '0.5rem 0 0 0' }}>
            Blog yazılarınızın kategorilerini yönetin
          </p>
        </div>
        ...
        {data?.categories.length === 0 ? (
          <div style={{ 
            ...
            {searchTerm 
              ...
              : 'Henüz blog kategorisi eklenmemiş'
            }
          </div>
        ) : (
          <>
            ...
            {data?.categories.map((category, index) => (
              <div
                ...
                style={{
                  ...
                  gap: '1rem',
                  padding: '1rem',
                  borderBottom: index < data.categories.length - 1 ? '1px solid #374151' : 'none',
                  alignItems: 'center'
                }}
              >
                <div>
